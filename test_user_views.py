...
class UserViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.testuser = User.signup(username="testname",
    ...
    def test_users_index(self):
        with self.client as c:
            ...
            self.assertIn("@testname", str(resp.data))
    ...
    def setup_likes(self):
        m1 = Message(text="big warble", user_id=self.testuser_id)
        m2 = Message(text="Pie yummy", user_id=self.testuser_id)
        m3 = Message(id=6666, text="I like warble", user_id=self.u1_id)
        db.session.add_all([m1, m2, m3])
        db.session.commit()

        l1 = Likes(user_id=self.testuser_id, message_id=6666)
    ...
    def test_add_like(self):
        m = Message(id=1984, text="Big Brother is watching", user_id=self.u1_id)
        db.session.add(m)
        db.session.commit()
    ...
    def test_remove_like(self):
        ...
        m = Message.query.filter(Message.text=="I like warble").one()
        ...
        # testuser now likes the message "warble"
        ...
        with self.client as c:
            ...
            self.assertEqual(resp.status_code, 200)
            # (ask mentor about the line of code below deletes the likes)
    ...
    def test_user_show_with_follows(self):
        ...
        with self.client as c:
            ...
            self.assertIn("@testname", str(resp.data))
            # Don't understand beautiful soup and the code below
            soup = BeautifulSoup(str(resp.data), 'html.parser')
            found = soup.find_all("li", {"class": "stat"})
            self.assertEqual(len(found), 4)
